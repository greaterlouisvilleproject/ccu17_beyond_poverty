...
Read in data
```{r}
data <- read_feather("housing.feather")
```

Subset to peers, apply city names, and remove residents of group quarters
```{r}
data$MET2013[data$PUMA == 100]  <- 46140
data$MET2013[data$PUMA == 1201] <- 46140
data$MET2013[data$PUMA == 1202] <- 46140
data$MET2013[data$PUMA == 1203] <- 46140
data$MET2013[data$PUMA == 1204] <- 46140
data$MET2013[data$PUMA == 1301] <- 46140
data$MET2013[data$PUMA == 1302] <- 46140
data$MET2013[data$PUMA == 1501] <- 46140
data$MET2013[data$PUMA == 1601] <- 46140

data <- data %>% rename(MSA = MET2013)

data <- pull_peers_MSA(data)

data$city<-NA
data$city[data$MSA == 24340] = "Grand Rapids"
data$city[data$MSA == 41180] = "St. Louis"
data$city[data$MSA == 36420] = "Oklahoma City"
data$city[data$MSA == 46140] = "Tulsa"
data$city[data$MSA == 24860] = "Greenville"
data$city[data$MSA == 28940] = "Knoxville"
data$city[data$MSA == 13820] = "Birmingham"
data$city[data$MSA == 31140] = "Louisville"
data$city[data$MSA == 26900] = "Indianapolis"
data$city[data$MSA == 28140] = "Kansas City"
data$city[data$MSA == 36540] = "Omaha"
data$city[data$MSA == 24660] = "Greensboro"
data$city[data$MSA == 16740] = "Charlotte"
data$city[data$MSA == 18140] = "Columbus"
data$city[data$MSA == 17140] = "Cincinnati"
data$city[data$MSA == 34980] = "Nashville"
data$city[data$MSA == 32820] = "Memphis"
data$city[data$MSA == 27260] = "Jacksonville"
data$city[data$MSA == 39580] = "Raleigh"
data$city[data$MSA == 19380] = "Dayton"
data$city[data$MSA == 40060] = "Richmond"

data <- data %>% filter(GQ == 1 | GQ == 2 | GQ == 5)
```
...
```{r}
data <- data %>% 
  group_by(YEAR, SERIAL, DATANUM) %>%
  mutate(max = max(MOVEDIN)) %>%
  filter(AGE < 18) %>%
  filter(YEAR > 2011)

data$moved <- if_else(data$max == 1, 1, 0)
data$moved[data$max == 0] <- NA

```

Assigns a value of 1 to the variable POV if the family is below the poverty line, otherwise 0. Values of 0 in the IPUMS POVERTY variable 
[signify NA.
```{r}
data$POV <- if_else(data$POVERTY <= 100, 1, 0)
data$POV[data$POVERTY == 0] <- NA
```

Creating a survey object from the housing data and a 
```{r}
housing_svy <- svydesign(ids = ~1, weights = ~PERWT, data = data)

housing_svy_df <- svyby(~moved, ~city+POV, design = housing_svy, svymean, na.rm = TRUE)
```
...
Read in data
```{r}
insecurity = read_feather('insecurity.feather')
```

The FAMINC variable is coded into categories that occur in intervals of 2500, 5000, and 10000. Here income is recoded to the lower value of the faminc variable.
```{r}
...
The same household is included in the CPS multiple times, so multiple observations of the same household are removed.
```{r}
...
```{r}
insecurity %<>% add_count(CPSID) %>% filter(YEAR>2011)
```
...
Create survey data frames. Use WTSUPP to weight the mean because the statistics produced are for the percentage of children who live in households that are food insecure and the percentage of children who are food insecure. 
```{r}
...
Sample size:
```{r}
table(insecurity$YEAR[insecurity$city == "Louisville"])
```

ASEC method - very small sample size but more precise poverty estimates

Read in the ASEC poverty supplement to the CPS
```{r}
poverty = read_feather('poverty.feather')
insecurity = read_feather('insecurity.feather')
```
...
Join the ASEC and the food insecurity supplement
```{r}
data = left_join(insecurity, poverty, by = c("CPSID", "CPSIDP")) %>%
  select(city, CPSID, CPSIDP, FSPOOR, YEAR = YEAR.x, AGE = AGE.x, HWTSUPP = HWTSUPP.x, WTSUPP = WTSUPP.x,
         OFFPOV, FSSTATUS, FSSTATUSC)
```

Generate dummy variables to describe whether the family is in poverty and is food secure
```{r}
data$poverty <- if_else(data$OFFPOV == 1, 1, 0)
data$poverty[data$OFFPOV == 99] <- NA

data$food_insecure <- if_else(data$FSSTATUS == 2 | data$FSSTATUS == 3, 1, 0)
data$food_insecure[data$FSSTATUS == 98 | data$FSSTATUS == 99] <- NA
```

Remove multiple observations of the same individuals from the data and subset to children
```{r}
data <- data[!duplicated(data$CPSIDP),]
...
Create survey data frames. Use WTSUPP to weight the mean because the statistics produced are for the percentage of children who live in households that are food insecure and the percentage of children who are food insecure. 
```{r}
fs_svy2 <- svydesign(ids = ~1, weights = ~WTSUPP, data = data)

fs_svy_df2 <- svyby(~food_insecure, ~city+poverty, design = fs_svy2, svymean, na.rm = TRUE)
```

Sample size:
```{r}
table(data$YEAR[data$city == "Louisville"])
```
